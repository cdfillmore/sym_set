...
# Szymon Rusinkiewicz
def evolute_3d(pts, v_norms, tris, thresh):
    ...
    for tri in tris:
        ...
        #break
        for vert in tri:
            u_p, v_p = v_coords[vert][0], v_coords[vert][1]
...
# first curve goes to pts additional curves can be added to args
def plot_medial_evolute(pts, lambda_value, pt, outfile=None, *args):
    extra_curves = [ar for ar in args]
    ...
    ax2.scatter(diag0.T[0], diag0.T[1], color='b', label='0th Diagram')
    ax2.scatter(diag1.T[0], diag1.T[1], color='r', label='1st Diagram')
    ax2.legend()
...
# make animation
dgms_all = []
for i,x in enumerate(pts_circ):
    print(i)
    dgms_all += [plot_medial_evolute(ellipse_big, 0.1, x, './anim3/frame_{}.png'.format(str(i).zfill(4)), ellipse_small)]
#os.system("ffmpeg -framerate 30 -pattern_type glob -i './anim3/*.png' -c:v libx264 -pix_fmt yuv420p extended_persistence.mp4")
...
max_diag = max(max_births, max_deaths)
flat_dgms_all = [[item for sublist in flat_dgms_all for item in sublist] for dgm in flat_dgms_all]

flatter_dgms_all = [item for sublist in flat_dgms_all for item in sublist]
diag0 = np.array([ i[1] for i in flatter_dgms_all if i[0]==0])
diag1 = np.array([ i[1] for i in flatter_dgms_all if i[0]==1])


plt.plot([0, max_diag], [0, max_diag], color='k', label = 'Diagonal')
plt.scatter(diag0.T[0], diag0.T[1], color='b', label='0th Diagram')
plt.scatter(diag1.T[0], diag1.T[1], color='r', label='1st Diagram')
plt.legend()
...
plt.grid(True)
plt.legend()
#plt.set_xlim([-0.1, max_diag])
#plt.set_ylim([-0.1, max_diag])
plt.show()
