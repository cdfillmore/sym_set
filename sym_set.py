...
def plot_medial_evolute(pts, lambda_value, pt, EXT=True, outfile=None, PD_plot=True, max_dist=1e10, col0='b', col1='r', *args):
    ...
    #evos += [evolute(pts, 1e-2, plot=False)]
    ...
    for evo in evos:
        ...
        #ax1.scatter(evo.T[0], evo.T[1], color='k', marker='x')
    '''
    ...
    if PD_plot:
        ...
        if EXT==True:
            flat_dgms = plot_extended_persistence3d(pts, simps, pt)
            max_births, max_deaths = max([ i[1][0] for i in flat_dgms]), max([ i[1][1] for i in flat_dgms])
        else:
            flat_dgms = plot_regular_persistence3d(pts, simps, pt)
            max_births = max([ i[1][0] for i in flat_dgms])
            max_deaths = sorted([ i[1][1] for i in flat_dgms if i[1][1] != np.inf])
            if len(max_deaths)==0:
                max_deaths = max_births
            else:
                max_deaths = max_deaths[-1]

        max_diag = max(max_births, max_deaths)
        diag0 = np.array([ i[1] for i in flat_dgms if i[0]==0])
        diag0[diag0==np.inf] = 1.02*max_diag
        diag1 = np.array([ i[1] for i in flat_dgms if i[0]==1])
        diag1[diag1==np.inf] = 1.02*max_diag



        #gudhi.plot_persistence_diagram(cplx.persistence(), axes=ax2)
        ax2.plot([0, max_diag], [0, max_diag], 'k', label = 'Diagonal')
        if EXT==False:
            ax2.plot([0, max_diag], [max_diag, max_diag], 'k--', label = 'Diagonal')
...
def plot_regular_persistence3d(pts, simps, pt, plotter=False, outfile=None, col0='b', col1='r', col2='g'):
    if len(simps) > 0:
...
# build extended Persistence diagram
def plot_extended_persistence3d(pts, simps, pt, plotter=False, outfile=None, col0='b', col1='r', col2='g'):
    if len(simps) > 0:
        d = max([len(i) for i in simps])
    else:
        d = 1
